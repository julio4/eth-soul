...
const CategoryBadge: React.FC<CategoryBadgeProps> = ({ badgeInfo }) => {
    ...
    return (
        <Box backgroundColor={badgeInfo.color} borderRadius="full" px={3} py={1} mr={2} mb={2} display="inline-block" boxShadow="md">
            <Text fontWeight={"bold"} fontSize={"sm"} color={textColor}>{badgeInfo.text}</Text>
        </Box>
    )
}
