		...
		<ZoomContext.Provider value={{ zoomLevel, setZoomLevel }}>
			<div tw="absolute flex min-h-full min-w-full flex-col">
				<main tw="relative flex grow flex-col">
					<Wrapper apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY as string} render={render}>
						<Map
							center={center}
							zoom={zoom}
							minZoom={14}
						...
						{highlightedMarker && <DetailedOffer offer={highlightedMarker} isOpen={isOpen} onClose={onClose} />}
					</Wrapper>
				</main>
			</div>
