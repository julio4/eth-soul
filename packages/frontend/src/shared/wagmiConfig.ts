...
const allChains: Chain[] = [mainnet, optimism, scrollTestnet, sepolia, localhost]

export const defaultChain: Chain | undefined = allChains.find((chain) => env.defaultChain === chain.id)
...
} = configureChains(Array.from(new Set([mainnet, defaultChain, ...supportedChains])).filter(Boolean) as Chain[], [
	jsonRpcProvider({
		rpc: (chain) => {
