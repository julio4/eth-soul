...
contract Sel is ERC20 {
    ...
    mapping(address => mapping(address => bool))
        private _boostedInteractionUsed;

    uint256 private constant inflationBuyerDiscount = 25;
    uint256 private constant inflationSellerBonus = 35;
    ...
    function getOffer(
        uint256 _offer
    ) public view returns (bytes32[2] memory, uint256, address, bool) {
        Offer storage offer = offers[_offer];
        return (
            [offer.hash1, offer.hash2],
            offer.value,
            offer.offerer,
            offer.active
        );
    ...
    function _applyTaxIfNecessary(address user) internal {
        ...
        if (_userActivities[user] > 1) {
            uint256 tax = (_taxConstants[weekInactives - 1] / 100) *
                balanceOf(user);
            _burn(user, tax);
        }
    }
    ...
    function cancelOffer(
        uint256 _offerId
    ) public payable offerExistance(_offerId) {
        Offer memory offer = offers[_offerId];
    ...
    function makeProposition(
        ...
        require(
            ...
            "You can't request your own offer"
        );

        proposers[msg.sender] = _offerId;

        emit PropositionMade(
            _offerId,
            msg.sender,
            offer.value,
            [offer.hash1, offer.hash2]
    ...
    function acceptOffer(
        ...
        require(offer.offerer == msg.sender, "You are not the offerer");
        require(
            proposers[proposer] == _offerId,
            "The provided address is not a proposer"
        );

        offer.active = false;
        emit PropositionAccepted(
            _offerId,
            offer.offerer,
            proposer,
            offer.value,
        ...
        if (!_boostedInteractionUsed[msg.sender][proposer]) {
            uint256 buyerAmount = (offer.value *
                (100 - inflationBuyerDiscount)) / 100;
            uint256 sellerAmount = (offer.value *
                (100 + inflationSellerBonus)) / 100;
